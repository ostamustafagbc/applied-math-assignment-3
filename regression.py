...
def fit_regression_model(X, y):
    ...
    for epoch in range(1, num_epochs):
        ...
        if abs(previos_loss - loss.item()) < 0.0005: # Change this condition to stop the training when the loss is not changing much.
            break
        previos_loss = loss.item()
        # This is a good place to print the loss every 1000 epochs.
        if epoch % 1000 == 0:
            print(f"epoch: {epoch}, loss: {previos_loss}")
    return model, loss
